...
# ORSU Active Directory Lab build script 
# DomainController (ORSU-DC) and Both Workstation (WORKSTATION01 & WORKSTATION02)  
#  -- Autoconfigured IP Addresses --
#  DC will always have ip x.x.x.250
#  WORKSTATION01 will always have ip x.x.x.220 
#  WORKSTATION02 will always have ip x.x.x.221
...
function fix_setspn {
  ...
  #setspn -D SQLService/Orsuevil.Corp ORSU-DC > $null
  #setspn -D SQLService/Orsuevil.Corp ORSUEVIL\SQLService > $null
  #setspn -D ORSU-DC/SQLService.Orsuevil.Corp:60111 ORSUEVIL\SQLService > $null
  #setspn -D ORSUEVIL/SQLService.Orsuevil.Corp:60111 ORSUEVIL\SQLService > $null
  #setspn -D DomainController/SQLService.Orsuevil.Corp:60111 ORSUEVIL\SQLService > $null
...
#--- new code 
  ...
  # setspn -A ORSU-DC/SQLService.Orsuevil.Corp:60111 ORSUEVIL\SQLService > $null
  # setspn -A SQLService/Orsuevil.Corp  ORSUEVIL\SQLService > $null
  # setspn -A DomainController/SQLService.Orsuevil.Corp:60111 ORSUEVIL\SQLService > $null
 ...
 setspn -A DomainController/SQLService.$FullDomainName`:60111 $ShortDomainName\SQLService > $null
  ...
  write-host("`n  [++] Checking Local Orsu-DC SPN")
  #setspn -L ORSU-DC
  # -- new code 
  setspn -L $machine 
  write-host("`n  [++] Checking ORSUEVIL\SQLService SPN")
  #setspn -L ORSUEVIL\SQLService
  # -- new code 
...
function build_lab {
  ...
  # install adds 
  write-host("`n  [++] Installing ADDS Domain : Orsuevil.corp ")
  Install-ADDSDomain -SkipPreChecks -ParentDomainName ORSUEVIL -NewDomainName corp -NewDomainNetbiosName ORSUEVIL `
  ...
  # create adds forest ORSUEVIL.corp
  write-host("`n  [++] Deploying Active Directory Domain Forest in ORSUEVIL.corp")
  Install-ADDSForest -SkipPreChecks -CreateDnsDelegation:$false -DatabasePath "C:\Windows\NTDS" `
  -DomainMode "WinThreshold" -DomainName "ORSUEVIL.corp" -DomainNetbiosName "ORSUEVIL" `
...
function get_sharphound {
  ...
  mkdir C:\ORSU\Sharphound
  $destination_path = "C:\ORSU\Sharphound"
...
function create_labcontent {
  ...
  # create user ammulu
  New-ADUser -Name "Ammulu" -GivenName "Ammulu" -Surname "Orsu" -SamAccountName "ammulu" `
  -UserPrincipalName "ammulu@$Global:Domain -Path DC=orsuevil,DC=corp" `
  ...
  Write-Host "`n  [++] User: Ammulu Orsu added, Logon: ammulu Password: Password2"
  Write-Host "        Adding Ammulu Orsu to Orsuevil.corp Groups: Domain Users"

  # create user vamsi
  New-ADUser -Name "Vamsi Krishna" -GivenName "Vamsi" -Surname "Krishna" -SamAccountName "vamsi" `
  -UserPrincipalName "vamsi@$Global:Domain -Path DC=orsuevil,DC=corp" `
  -AccountPassword (ConvertTo-SecureString "Password1" -AsPlainText -Force) `
  -PasswordNeverExpires $true -PassThru | Enable-ADAccount  | Out-Null

  # if the rps_s_access_denited is fixed by the reg key, vamsi no longer needs to be a domain admin
  Add-ADGroupMember -Identity "Domain Admins" -Members vamsi  | Out-Null
  Write-Host "`n  [++] User: Vamsi Krishna added, Logon: vamsi Password: Password1"
  Write-Host "        Adding Vamsi Krishna to Orsuevil.corp Groups: Domain Users, Domain Admins"

  # create user tstark 
  New-ADUser -Name "Adolf Hitler" -GivenName "Adolf" -Surname "Hitler" -SamAccountName "hitler" `
  -UserPrincipalName "hitler@$Global:Domain -Path DC=orsuevil,DC=corp" `
  -AccountPassword (ConvertTo-SecureString "Password2019!@#" -AsPlainText -Force) `
  -PasswordNeverExpires $true -PassThru | Enable-ADAccount | Out-Null

  Add-ADGroupMember -Identity "Administrators" -Members hitler
  Add-ADGroupMember -Identity "Domain Admins" -Members hitler
  Write-Host "`n  [++] User: Adolf Hitler added, Logon: hitler Password: Password2019!@#"
  Write-Host "        Adding Adolf Hitler to Orsuevil.corp Groups: Administrators, Domain Admins"

  # create user sqlservice 
  New-ADUser -Name "SQL Service" -GivenName "SQL" -Surname "Service" -SamAccountName "sqlservice" `
  -UserPrincipalName "sqlservice@$Global:Domain -Path DC=orsuevil,DC=corp" `
  ...
  Write-Host "        Adding SQLService to Orsuevil.corp Groups: Administrators, Domain Admins, Enterprise Admins, Group Policy Creator Owners, Schema Admins"
  ...
  # create ou=groups, move all existing groups into ou=groups,dc=orsuevil,dc=corp
  New-ADOrganizationalUnit -Name "Groups" -Path "DC=ORSUEVIL,DC=CORP" -Description "Groups" | Out-Null
  get-adgroup "Schema Admins" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "Allowed RODC Password Replication Group" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "Cert Publishers" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "Cloneable Domain Controllers" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "Denied RODC Password Replication Group" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "DnsAdmins" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "DnsUpdateProxy" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "Domain Computers" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "Domain Controllers" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "Domain Guests" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "Domain Users" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "Domain Admins" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "Enterprise Admins" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "Enterprise Key Admins" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "Enterprise Read-only Domain Controllers" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "Group Policy Creator Owners" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "Key Admins" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "Protected Users" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "RAS and IAS Servers" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  get-adgroup "Read-only Domain Controllers" | move-adobject -targetpath "OU=Groups,DC=ORSUEVIL,DC=CORP" | Out-Null
  }
  # ---- end create_labcontent function


  # ---- begin create_orsuevil_gpo
function create_orsuevil_gpo {
  ...
  Get-GPO -Name "Disable Defender" | Remove-GPLink -target "DC=$CurrentDomain,DC=corp" | Remove-GPO -Name "Disable Defender" > $null 
  ...
  Get-GPO -Name "Disable Defender" | New-GPLink -target "DC=ORSUEVIL,DC=corp" -LinkEnabled Yes -Enforced Yes
...
# ---- begin set_workstation01_staticip function  
function set_workstation01_staticip { 
  # get the ip address
  $IPAddress=Get-NetIPAddress -AddressFamily IPv4 -InterfaceIndex $(Get-NetConnectionProfile | Select-Object -ExpandProperty InterfaceIndex) | Select-Object -ExpandProperty IPAddress
  ...
  # ---- end set_workstation01_staticip function  

# ---- begin set_workstation02_staticip function  
function set_workstation02_staticip { 
  # get the ip address
  $IPAddress=Get-NetIPAddress -AddressFamily IPv4 -InterfaceIndex $(Get-NetConnectionProfile | Select-Object -ExpandProperty InterfaceIndex) | Select-Object -ExpandProperty IPAddress
  ...
  # ---- end set_workstation02_staticip function
...
function fix_workstationdns {
  $DCDNS=(Test-Connection -comp ORSU-DC -Count 1).ipv4address.ipaddressToString
  
  write-host("`n  [++] Found ORSU-DC At $DCDNS")
  $adapter=Get-CimInstance -Class Win32_NetworkAdapter -Property NetConnectionID,NetConnectionStatus | Where-Object { $_.NetConnectionStatus -eq 2 } | Select-Object -Property NetConnectionID -ExpandProperty NetConnectionID
...
function server_build {
  ...
  if($currentname -ne "ORSU-DC") {
      write-host("`n  Computer Name is Incorrect Setting ORSU-DC")
      write-host("`n  - Script Run 1 of 3 - Setting the computer name to ORSU-DC and rebooting")
      ...
      set_dcstaticip
      Rename-Computer -NewName "ORSU-DC" -Restart
      }
      elseif ($domain -ne "ORSUEVIL.CORP") {
      ...
      elseif ($domain -eq "ORSUEVIL.CORP" -And $machine -eq "ORSU-DC") {
...
function git_recon() { 
  ...
  write-host("`n  [++] Downloading Powershell Mafia v1.9 to C:\orsu")
  mkdir $HOME\Documents\WindowsPowerShell\Modules\Recon
  git clone https://github.com/PowerShellMafia/PowerSploit C:\orsu\PowerShellMafia
  write-host("`n  [++] Copying Recon to C:\$HOME\Documents\WindowsPowerShell\Modules\Recon")
  echo D | xcopy /e /y C:\orsu\PowerShellMafia\Recon $HOME\Documents\WindowsPowerShell\Modules\Recon
  }
...
function workstations_common { 
  ...
  mkdir C:\ORSU > $null 
  write-host("`n  [++] Downloading Powerview v1.9 to C:\ORSU")
  Invoke-WebRequest  https://raw.githubusercontent.com/PowerShellEmpire/PowerTools/version_1.9/PowerView/powerview.ps1 -o C:\ORSU\Powerview.ps1 | Out-Null
  ...
  # download an unzip Sharphound.zipi to C:\ORSU\Sharphound
  get_sharphound 

  # download and unzip pstools.zip to c:\pstools 
  write-host("`n  [++] Downloading PSTools to C:\ORSU")
  Invoke-WebRequest  https://download.sysinternals.com/files/PSTools.zip -o C:\ORSU\PStools.zip | Out-Null
  Start-BitsTransfer -Source "https://download.sysinternals.com/files/PSTools.zip" -Destination "C:\ORSU\PSTools.zip" | Out-Null
  write-host("`n  [++] Extracting PSTools to C:\PSTools")
  Expand-Archive -Force C:\ORSU\PSTools.zip C:\PSTools | Out-Null 
  ...
  write-host("`n Joining machine to domain Orsuevil.corp")
  # add-computer -domainname "ORSUEVIL.CORP" -username administrator -restart | Out-Null
  $domain = "ORSUEVIL"
...
# ---- begin workstation_workstation01 function 
function workstation_workstation01 { 
  write-host("`n`n   Computer Name : $machine")
  write-host("     Domain Name : $domain")
  write-host("      OS Version : $osversion")

  if ($machine -ne "WORKSTATION01") { 
    write-host ("`n Setting the name of this machine to WORKSTATION01 and rebooting automatically...")
    write-host (" Run this script 1 more time and select 'P' in the menu to join the domain")
    Read-Host -Prompt "`n Press ENTER to continue..."
    # set_mppref
    set_workstation01_staticip 
    Rename-Computer -NewName "WORKSTATION01" -Restart
    }
    elseif ($machine -eq "WORKSTATION01") {
      workstations_common
      ...
      Read-Host -Prompt "`n All done! $machine is all setup! `n Press Enter to reboot and Login as ORSUEVIL\vamsi and Password1 "
      restart-computer 
    }
    else { write-host("Nothing to do here") }
    } 
    # ---- end workstation_workstation01 function 
    
# ---- begin workstation_workstation02 function
function workstation_workstation02 { 
  write-host("`n`n   Computer Name : $machine")
  write-host("     Domain Name : $domain")
  write-host("      OS Version : $osversion")
  
  if ($machine -ne "WORKSTATION02") {
    write-host ("`n Setting the name of this machine to WORKSTATION02 and rebooting automatically...")
    write-host (" Run this script 1 more time and select 'S' in the menu to join the domain")
    Read-Host -Prompt "`n Press ENTER to continue..."
    set_mppref
    set_workstation02_staticip
    Rename-Computer -NewName "WORKSTATION02" -Restart
    }
    elseif ($machine -eq "WORKSTATION02") {
      workstations_common 
      Get-LocalUser -Name "Administrator" | Enable-LocalUser
      $UserAccount = Get-LocalUser -Name "Administrator"
      $UserAccountPassword = "Password1" | ConvertTo-SecureString -asPlainText -Force
      $UserAccount | Set-LocalUser -Password $UserAccountPassword
      #add vamsi as a local administrator on the workstation02 machine 
      Add-LocalGroupMember -Group Administrators -Member Vamsi -Verbose
      Read-Host -Prompt "`n All done! $machine is all setup! `n Press Enter to reboot and Login as ORSUEVIL\ammulu and Password2 "
      restart-computer 
      }
    else { write-host("Nothing to do here") }
    } 
    # ---- end workstation_workstation02 function

# ---- begin menu function
function menu {
  do {
    Write-Host "`n`n`t ORSU AD-Lab Build Menu - Select an option`n"
    Write-Host "`tPress 'D' to setup ORSU-DC Domain Controller"
    Write-host "`t(must be run 3 times)`n"
    Write-Host "`tPress 'P' to setup WORKSTATION01 and join the domain Orsuevil.Corp"
    Write-host "`t(must be run 2 times)`n"
    Write-Host "`tPress 'S' to setup WORKSTATION02 and join the domain Orsuevil.Corp" 
    Write-host "`t(must be run 2 times)`n"
    ...
    Write-Host "`n`tPress 'H' to only download sharphound.zip and extract to C:\ORSU\Sharphound"
  ...
  switch ($choice) {
    'D'{  Write-Host "`n Running... ORSU-DC domain controller"
          nukedefender 
          server_build }
    'P'{  Write-Host "`n Running... WORKSTATION01"
          nukedefender 
          workstation_workstation01 }
    'S'{  Write-Host "`n Running... WORKSTATION02"
          nukedefender 
          workstation_workstation02 }
    ...
    'A'{  Write-Host "`n ONLY running... Fix ADCSCertificateAuthority Function and exit"
          fix_adcsca }    
    'H'{  Write-Host "`n ONLY running... Download Sharphound.zip and extract to c:\ORSU"
          get_sharphound }                           
    'X'{Return}
    }
  } 
